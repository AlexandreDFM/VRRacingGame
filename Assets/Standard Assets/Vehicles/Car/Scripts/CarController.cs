...
namespace UnityStandardAssets.Vehicles.Car {
    ...
    public class CarController : MonoBehaviour {
        ...
        private static readonly int NoOfGears = 5;
        ...
        private void Update() {
            if (Player) {
                ...
                if (SaveScript.BreakSlide)
                    MySteerHelper = 0.9f;
                else
                    MySteerHelper = m_SteerHelper;
            }
        }
                ...
                if (Player) {
                    if (SaveScript.OnTheTerrain)
                        if (wheelHit.collider.CompareTag("Road")) {
                            SaveScript.OntTheRoad = true;
                            SaveScript.OnTheTerrain = false;
                        }

                    if (SaveScript.OntTheRoad)
                        if (wheelHit.collider.CompareTag("Terrain")) {
                            SaveScript.OntTheRoad = false;
